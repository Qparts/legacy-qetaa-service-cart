		...
		try {
			...
			for (Integer makeId : makeIds) {
				...
				for (Calendar c : calendars) {
					...
					String sql = "select count(*) from crt_cart b where b.make_id = "+makeId+ " and to_char(b.created, 'YYYY-MM') = '"+year + "-" + monthString+"'";
					...
					String sql2 = "select count(*) from crt_parts_order b where b.cart_id in ("
							+ "select c.id from crt_cart c where c.make_id = "+makeId+ " and to_char(c.created, 'YYYY-MM') = '"+year + "-" + monthString+"')";
